...
# --- load data---
training_data                <- read.csv("HPP_train/predict_sigma/training.csv",
...
save(model, file = "HPP_train/predict_sigma/model.RData")
model_p <- data.frame(metafeatures = metafeatures, means = means, scales = scales,
                      n_boot = 50 , percentage  = 0.95, enableLog = 0, step = 0.1, count = 0)
write.csv(model_p, "HPP_train/predict_sigma/model_parameters.csv")

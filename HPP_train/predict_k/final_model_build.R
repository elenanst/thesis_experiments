...
# --- load data---
training_data                <- read.csv("HPP_train/predict_k/training.csv",
...
for(i in seq(1, nrow(metafeatures))) {
  ...
  testData$Class <- NULL
  trained_model  <- caret::train((Class) ~ ., data = trainData,
                                 method = "ranger", 
                                 trControl=fitControl, keep.inbag = TRUE)
  predictions    <- boot_pi(model = trained_model, pdata = testData, n = 50, p = 0.98, enableLog = FALSE)
...
tikz('HPP_train/predict_deacay/intervals.tex', standAlone = TRUE, width=5, height=5)
...
model   <- caret::train((Class) ~ ., data = training_data,
                        method = "ranger", 
                        trControl=fitControl)
save(model, file = "HPP_train/predict_k/model.RData")
model_p <- data.frame(metafeatures = metafeatures, means = means, scales = scales,
                      n_boot = 50 , percentage  = 0.95, enableLog = 0, step = 1, count = 1)
write.csv(model_p, "HPP_train/predict_k/model_parameters.csv")

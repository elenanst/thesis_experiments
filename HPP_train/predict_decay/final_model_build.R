...
zero_columns  <- nearZeroVar(training_data, freqCut = 5, uniqueCut = 70, saveMetrics =F)
...
counts
training_data<-as.data.frame(training_data[,counts>1, drop = FALSE])
...
indexes <- which(test_class>=1)
tikz('HPP_train/predict_decay/intervals.tex', standAlone = TRUE, width=5, height=5)
...
axis(1,at=1:23)
axis(2,at=seq(0,max(c(test_class[-indexes], ci_high[-indexes]))+0.01,0.05))
...
# --- save workspace
save(list = ls(all.names = TRUE), file = "HPP_train/predict_decay/workspace.RData", envir = .GlobalEnv)

# --- train and store model and parameters ---
save(trained_model, file = "HPP_train/predict_decay/model.RData")
model_p <- list(metafeatures = colnames(training_data), means = means, scales = scales,
                n_boot = 50 , percentage  = 0.95, enableLog = 1, step = 0.2, count = 0)
save(model_p, file="HPP_train/predict_decay/model_parameters.Rdata")

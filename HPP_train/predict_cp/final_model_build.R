# Trains model for predicting cp of rpart.
...
class                        <- read.csv("data/optimization/bayesian/tree_opt.csv",
                                         header = TRUE, sep=",", stringsAsFactors=FALSE)
...
counts
training_data<-as.data.frame(training_data[, counts>=(trials/10)])
...
indexes        <- which(class > 1)
trained_model  <- caret::train(log10(Class) ~ ., data = training_data[-indexes,],
                               method = "ranger", 
...
tikz('HPP_train/predict_cp/intervals.tex', standAlone = TRUE, width=5, height=5)

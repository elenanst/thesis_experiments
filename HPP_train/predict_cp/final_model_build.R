...
# --- load data---
training_data                <- read.csv("HPP_train/predict_cp/training.csv",
...
# --- save workspace
save(list = ls(all.names = TRUE), file = "train_HPP/predict_cp/workspace.RData", envir = .GlobalEnv)

# --- train and store model and parameters ---
model   <- caret::train(log10(Class) ~ ., data = training_data,
                        method = "ranger", 
                        trControl=fitControl)
save(model, file = "HPP_train/predict_cp/model.RData")
...
write.csv(model_p, "HPP_train/predict_cp/model_parameters.csv")

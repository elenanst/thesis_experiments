...
# computed for the size-HPP model) and the performance of our system.
#
...
write.csv(dataset,"readiness_metric/data/distances_size.csv")
ggplot(dataset, aes(x = X)) + geom_histogram(aes(y = ..density..)) + geom_density(adjust = 1.4,kernel = "gaussian")
density_estimate <- density(distance_scores,adjust = 1.4,kernel = "gaussian")
first_q          <- 2.1875
third_q          <- 7.0288

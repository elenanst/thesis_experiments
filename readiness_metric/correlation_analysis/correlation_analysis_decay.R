...
# --- load data ---
# HPP model and data
load("../automl/workspace/HPP/models/AnnClassifier/decay/model.RData")
hpp_data  <- read.csv("../automl/workspace/HPP/models/AnnClassifier/decay/parameters.csv",
...
write.csv(data.frame(distances = distance_scores), "distances_decay.csv")
dataset <- data.frame(X= distance_scores)
library(tikzDevice)
tikz('../automl_experiments/distances_hist_decay.tex', standAlone = TRUE, width=7, height=5)
ggplot(dataset, aes(x = X)) + geom_histogram(aes(y = ..density..)) + geom_density(adjust = 2,kernel = "gaussian")
dev.off()
density_estimate <- density(distance_scores,adjust=2,kernel = "gaussian")
first_q          <- 2.7418
third_q          <- 9.8111

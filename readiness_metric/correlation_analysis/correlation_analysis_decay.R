...
write.csv(dataset,"readiness_metric/data/distances_decay.csv")
ggplot(dataset, aes(x = X)) + geom_histogram(aes(y = ..density..)) + geom_density(adjust = 1.8,kernel = "gaussian")
density_estimate <- density(distance_scores,adjust=1.8,kernel = "gaussian")
first_q          <- 3.001
third_q          <- 10.283

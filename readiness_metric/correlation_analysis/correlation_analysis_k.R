...
# class of hp
test_class <- read.csv("data/opt/k_grid.csv",
                       header = TRUE, sep=",", stringsAsFactors=FALSE)
test_class <- test_class[test_class$name %in% metafeatures_test$X,]
...
write.csv(data.frame(distances = distance_scores), "distances_k.csv")
dataset <- data.frame(X= distance_scores)
library(tikzDevice)
tikz('../automl_experiments/distances_hist_sigma.tex', standAlone = TRUE, width=7, height=5)
ggplot(dataset, aes(x = X)) + geom_histogram(aes(y = ..density..)) + geom_density(adjust = 1.1,kernel = "gaussian")
dev.off()
density_estimate <- density(distance_scores,adjust=1.1,kernel = "gaussian")
first_q          <- 5.19955
third_q          <- 15.65788

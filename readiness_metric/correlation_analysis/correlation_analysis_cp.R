...
# computed for the cp-HPP model) and the performance of our system.
#
...
test_class <- read.csv("data/optimization/bayesian/tree_opt.csv",
                       header = TRUE, sep=",", stringsAsFactors=FALSE)
...
write.csv(dataset,"readiness_metric/data/distances_cp.csv")
ggplot(dataset, aes(x = X)) + geom_histogram(aes(y = ..density..)) + geom_density(adjust = 1.2,kernel = "gaussian")
density_estimate <- density(distance_scores,adjust=1.2,kernel = "gaussian")
first_q          <- 2.1679
third_q          <- 7.1891

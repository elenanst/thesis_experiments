...
# --- load data ---
# HPP model and data
load("../automl/workspace/HPP/models/TreeClassifier/cp/model.RData")
hpp_data  <- read.csv("../automl/workspace/HPP/models/TreeClassifier/cp/parameters.csv",
...
# class of hp
test_class <- read.csv("data/opt/tree_alt_opt.csv",
                       header = TRUE, sep=",", stringsAsFactors=FALSE)
...
write.csv(data.frame(distances = distance_scores), "distances_cp.csv")
dataset <- data.frame(X= distance_scores)
library(tikzDevice)
tikz('../automl_experiments/distances_hist_cp.tex', standAlone = TRUE, width=7, height=5)
ggplot(dataset, aes(x = X)) + geom_histogram(aes(y = ..density..)) + geom_density(adjust = 0.6,kernel = "gaussian")
dev.off()
density_estimate <- density(distance_scores,adjust=0.6,kernel = "gaussian")
first_q          <- 2.0144
third_q          <- 6.6230
